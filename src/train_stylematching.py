...
def call_dataset(args, tokenizer, accelerator):
    ...
    train_transforms = transforms.Compose([transforms.CenterCrop(args.resolution) if args.center_crop else transforms.RandomCrop(args.resolution),
...
def main(args):
    ...
    new_id = 'water_reflection_applied_pix2pix'
    unet = UNet2DConditionModel.from_pretrained(new_id, subfolder="unet", revision=args.non_ema_revision)
    """
