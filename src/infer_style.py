...
def main(args) :
    ...
    for id in range(args.start_id, args.end_id,  50) :
        ...
        save_folder = os.path.join(args.output_dir, f"validation_{resume_from_checkpoint}_generality_test")
        if args.style_prompt != "" :
            save_folder = os.path.join(args.output_dir, f"validation_{resume_from_checkpoint}_with_style_prompt")
        #if args.concept_word is not None :
        #    save_folder = os.path.join(args.output_dir, f"validation_{resume_from_checkpoint}_with_concept_special_timestep_special_batch")
        ...
        for i, item in enumerate(datas):
            data_base = '../../../data/diffusion/ClimateDiffusion/TestData/'
            # 1) original image
            ...
            for p_idx, prompt in enumerate(prompt_list) :
                ...
                if class_label in args.class_label_list:
                    ...
                    for image_guidance_scale in image_guidance_scales :
                        ...
                        #if args.concept_word is not None:
                        #prompt = args.concept_word
                        #for structure_guidance_scale in structure_guidance_scales :
                        #args.structure_guidance_scale = structure_guidance_scale
