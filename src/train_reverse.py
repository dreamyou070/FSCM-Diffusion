    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with (accelerator.accumulate(unet)):
                ...
                class_labels = batch['labels'].to(device=accelerator.device)  # when  True
                ...
                bsz = latents.shape[0]
                timesteps = torch.randint(0, noise_scheduler.config.num_train_timesteps, (bsz,), device=latents.device) # random timestep
                ...
                if args.conditioning_dropout_prob is not None:
                    ...
                    class_mask = random_p < args.conditioning_dropout_prob # 0.1
                    # [1] uncon applied class condition
                    class_labels = batch["labels"].to(device=accelerator.device) # when False
                    neg_class_labels = batch['neg_labels'].to(device=accelerator.device) # when  True
                    class_labels = torch.where(class_mask, neg_class_labels, class_labels)

                    # [2] uncon applied text condition
                    prompt_mask = class_mask.reshape(bsz, 1, 1)
                    null_conditioning = text_encoder(tokenize_captions([""], tokenizer).to(accelerator.device))[0]
                    encoder_hidden_states = torch.where(prompt_mask,null_conditioning,encoder_hidden_states) # level 하고 같이 가도 좋을듯

                    # [3] uncon applied image condition
                ...
                if getattr(args, "reverse", False):  # reverse 옵션이 있을 경우
                    # Swap source and target
                    edited_pixel_values = batch["original_pixel_values"]
                    original_pixel_values = batch["edited_pixel_values"]
                    class_labels = batch['neg_labels'].to(device=accelerator.device)  # when  True
                else:
                    edited_pixel_values = batch["edited_pixel_values"]
                    original_pixel_values = batch["original_pixel_values"]

                # [2] Target latent (이전: edited image → target)
                latents = vae.encode(
                    edited_pixel_values.to(weight_dtype)).latent_dist.sample() * vae.config.scaling_factor

                # [3] Condition latent (이전: original image → condition)
                original_image_embeds = vae.encode(original_pixel_values.to(weight_dtype)).latent_dist.mode()
                ...
                model_pred = unet(concatenated_noisy_latents,
                                  ...
                                  return_dict=False, )[0]
                cond_loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean")

                # unconditional
                """
                neg_model_pred = unet(concatenated_noisy_latents,
                                                timesteps,
                                                encoder_hidden_states,
                                                added_cond_kwargs=added_cond_kwargs,
                                                class_labels=neg_class_labels,
                                                return_dict=False, )[0]
                neg_loss = F.mse_loss(neg_model_pred.float(), target.float(),
                                      reduction="mean")
                """
                #total_loss = cond_loss + neg_loss
